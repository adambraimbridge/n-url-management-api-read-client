...
function raceDynamos () {
	return surviver([
		get({ dynamo: dynamos.master.instance, table: dynamos.master.table, fromURL: 'https://www.ft.com/fastft', metrics: metrics }).then(() => 'master'),
		get({ dynamo: dynamos.slave.instance, table: dynamos.slave.table, fromURL: 'https://www.ft.com/fastft', metrics: metrics }).then(() => 'slave')
	])
		.then(fasterDynamo => {
