...
function raceDynamos () {
	return surviver([
		get({ dynamo: dynamos.master.instance, table: dynamos.master.table, fromURL: HEALTHCHECK_URL, metrics: metrics }).then(() => 'master'),
		get({ dynamo: dynamos.slave.instance, table: dynamos.slave.table, fromURL: HEALTHCHECK_URL, metrics: metrics }).then(() => 'slave')
	])
		.then(fasterDynamo => {
