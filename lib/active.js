...
function raceDynamos() {
	return surviver([
		get({ dynamo: dynamos.master.instance, table: dynamos.master.table, fromURL: 'www.ft.com/fastft', metrics: metrics, cache: false }).then(() => 'master'),
		get({ dynamo: dynamos.slave.instance, table: dynamos.slave.table, fromURL: 'www.ft.com/fastft', metrics: metrics, cache: false }).then(() => 'slave')
	])
		.then(fasterDynamo => {
