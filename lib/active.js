...
function raceDynamos() {
	return surviver([
		get(dynamos.master.instance, dynamos.master.table, 'www.ft.com/fastft', metrics).then(() => 'master'),
		get(dynamos.slave.instance, dynamos.slave.table, 'www.ft.com/fastft', metrics).then(() => 'slave')
	])
		.then(fasterDynamo => {
